...
namespace Monocle {
	public static class Calc {
		...
		public static T GetFromArray<T>(this T[ , ] _this, int _x, int _y) {
			if (_x < 0)
				_x = 0;
			else if (_x >= _this.GetLength(0))
				_x = _this.GetLength(0) - 1;
			if (_y < 0)
				_y = 0;
			else if (_y >= _this.GetLength(1))
				_y = _this.GetLength(1) - 1;
		...
		public static void AddString(this List<byte> _byteList, string _value) {
			_byteList.Add((byte)_value.Length);
			_byteList.AddRange(Encoding.ASCII.GetBytes(_value));
		}
		public static void AddFloat(this List<byte> _byteList, float _value) {
			_byteList.AddRange(BitConverter.GetBytes(_value));
		}
		public static void AddInt(this List<byte> _byteList, int _value) {
			_byteList.AddRange(BitConverter.GetBytes(_value));
		}
		public static void AddUInt(this List<byte> _byteList, uint _value) {
			_byteList.AddRange(BitConverter.GetBytes(_value));
		}
		public static void AddBool(this List<byte> _byteList, bool _value) {
			_byteList.Add((byte)((bool)_value ? 1 : 0));
		}
		...
		public static float LerpDelta(float value1, float value2, float lerp, float timeElapsed) {
			if (lerp <= 0)
				return value1;
			if (lerp >= 1)
				return value2;

			lerp = 1 - (float)Math.Pow(1 / (1 - lerp), -timeElapsed);
			return MathHelper.Lerp(value1, value2, lerp);
		}

		public static Vector2 LerpDelta(Vector2 value1, Vector2 value2, float lerp, float timeElapsed) {
			if (lerp <= 0)
				return value1;
			if (lerp >= 1)
				return value2;
			...
			lerp = 1 - (float)Math.Pow(1 / (1 - lerp), -timeElapsed);
			return Vector2.Lerp(value1, value2, lerp);
		}
		...
		public static float SnapFloor(float value, float increment) {
			return (float)Math.Floor(value / increment) * increment;
		}
		public static float SnapCeiling(float value, float increment) {
			return (float)Math.Ceiling(value / increment) * increment;
		}
		...
		public static Vector2 SnapFloor(this Vector2 value, float increment) {
			return new Vector2(SnapFloor(value.X, increment), SnapFloor(value.Y, increment));
		}
		public static Vector2 SnapCeiling(this Vector2 value, float increment) {
			return new Vector2(SnapCeiling(value.X, increment), SnapCeiling(value.Y, increment));
		}
		...
		public static Vector3 SnapFloor(this Vector3 value, float increment) {
			return new Vector3(SnapFloor(value.X, increment), SnapFloor(value.Y, increment), SnapFloor(value.Z, increment));
		}

		public static Vector3 SnapCeiling(this Vector3 value, float increment) {
			return new Vector3(SnapCeiling(value.X, increment), SnapCeiling(value.Y, increment), SnapCeiling(value.Z, increment));
		}
		...
		public static float AngleDiffCW(float radiansA, float radiansB) {
			float diff = radiansB - radiansA;

			while (diff > MathHelper.TwoPi) { diff -= MathHelper.TwoPi; }
			while (diff <= 0) { diff += MathHelper.TwoPi; }

			return diff;
		}
		public static float AngleDiffCCW(float radiansA, float radiansB) {
			return AngleDiffCW(radiansB, radiansA);
		}
		...
		public static float AngleDiff(Vector2 vectorA, Vector2 vectorB) {
			return AngleDiff(vectorA.Angle(), vectorB.Angle());
		}

		public static float AbsAngleDiff(Vector2 radiansA, Vector2 radiansB) {
			return Math.Abs(AngleDiff(radiansA, radiansB));
		}

		public static int SignAngleDiff(Vector2 radiansA, Vector2 radiansB) {
			return Math.Sign(AngleDiff(radiansA, radiansB));
		}
		...
		public static T[ , ] Rearrange<T>(this T[ ] _array, int _width) {
			T[ , ] retval = new T[_width, _array.Length / _width];

			for (int i = 0; i < _array.Length; ++i) {
				retval[i % _width, i / _width] = _array[i];
			}
		...
		public static float ClampSign(float value, float min, float max) {
			return Clamp(Math.Abs(value), min, max) * Math.Sign(value);
		}

		public static int ClampSign(int value, int min, int max) {
			return Clamp(Math.Abs(value), min, max) * Math.Sign(value);
		}
		...
		public static float ClampLoop(float _a, float _b, float _value) {
			_value -= _a;
			_value %= _b - _a;
			if (_value < 0) {
				_value = (_b - _a) + _value;
		...
		public static int SortByDepth(Entity a, Entity b) {
			return a.updateDepth - b.updateDepth;
		}

		public static int SortByDepthReversed(Entity a, Entity b) {
			return b.updateDepth - a.updateDepth;
		}
		...
		public static void SetParameter(this Effect effect, string parameter, Color value) {
			foreach (var p in effect.Parameters) {
				if (p.Name == parameter) {
					p.SetValue(new Vector4(value.R / 255f, value.G / 255f, value.B / 255f, value.A / 255f));
				}
			}
		}
