...
namespace Monocle {
	public class Camera {
		...
		private void UpdateMatrices() {
			Vector2 pos = position.SnapFloor(Engine.InvertScaling);
			Vector2 orig = origin.SnapFloor(Engine.InvertScaling);

			matrix = Matrix.Identity *
					Matrix.CreateTranslation(new Vector3(-new Vector2(pos.X, pos.Y), 0)) *
					Matrix.CreateRotationZ(angle) *
					Matrix.CreateScale(new Vector3(zoom, 1)) *
					Matrix.CreateTranslation(new Vector3(new Vector2(orig.X, orig.Y), 0));
