...
namespace Monocle {
	...
	public class VirtualButton : VirtualInput {
		...
		public override void Update() {
			...
			bufferCounter = Calc.Approach(bufferCounter, 0, Engine.DeltaTime);

			bool check = false;
			foreach (var node in Nodes) {
				node.Update();
				if (node.Pressed && bufferCounter >= 0) {
			...
			if (!check) {
				Repeating = false;
				repeatCounter = 0;
				bufferCounter = Math.Min(0, bufferCounter);
			}
			else if (canRepeat) {
				Repeating = false;
				if (repeatCounter == 0)
		...
		public bool Check {
			get {
				...
				if (MInput.Disabled || bufferCounter < 0)
					return false;
		...
		/// </summary>
		public void ConsumePress(float waitTime = 0) {
			bufferCounter = Math.Min(-waitTime, 0);
			consumed = true;
		}
		...
		public class PadButton : Node {
			...
			public override bool Check {
				get {
					if (GamepadIndex < 0) {
						foreach (var pad in MInput.GamePads) {
							if (pad.Check(Button))
								return true;
						}
						...
						return MInput.GamePads[GamepadIndex].Check(Button);
				}
			}
			...
			public override bool Pressed {
				get {
					if (GamepadIndex < 0) {
						...
						return MInput.GamePads[GamepadIndex].Pressed(Button);
				}
			}
		...
		public class BooleanButton : Node {
			...
			public BooleanButton(PropertyInfo _info, object _obj) {
			...
			public override void Update() {
				base.Update();
				previous = current;
				current = (bool)get.Invoke(rootObj, null);
			}
		}
