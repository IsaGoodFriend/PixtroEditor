	...
	public class EntityList : IEnumerable<Entity>, IEnumerable {
		...
		public void Add(Entity entity) {
			if (!adding.Contains(entity) && !current.Contains(entity)) {
				if (Scene != null && !entity.Added(Scene))
					return;
		...
		internal void Update() {
			foreach (var entity in entities)
				if (entity.Active && (entity.UpdateWhileFrozen || Engine.FreezeTimer <= 0))
					entity.Update();
		}
		...
		public void Render() {
			foreach (var entity in entities)
				if (entity.Visible) {
					Draw.WorldSecondary = Matrix.Identity;
					entity.Render();
				}
		}
		...
		public static Comparison<Entity> CompareDepth = (a, b) => { return Math.Sign(b.updateDepth - a.updateDepth); };
	}
}
