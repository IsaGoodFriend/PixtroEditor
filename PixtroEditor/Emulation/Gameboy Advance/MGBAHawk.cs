	...
	public partial class MGBAHawk : IEmulator
	{
		...
		static MGBAHawk()
		{
			var resolver = new DynamicLibraryImportResolver(
				OSTailoredCode.IsUnixHost ? "libmgba.dll.so" : "dll/mgba.dll", hasLimitedLifetime: false);
			LibmGBA = BizInvoker.GetInvoker<LibmGBA>(resolver, CallingConventionAdapters.Native);
		}

		public MGBAHawk(byte[] file)
		{
			_syncSettings = new SyncSettings();
			_settings = new Settings();
			DeterministicEmulation = false;

			byte[] bios = null; //comm.CoreFileProvider.GetFirmware("GBA", "Bios", false);
			...
			Core = LibmGBA.BizCreate(bios, file, file.Length, GetOverrideInfo(new GameInfo() { }), skipBios);
			if (Core == IntPtr.Zero)
			{
