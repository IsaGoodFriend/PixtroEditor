...
namespace Pixtro.Emulation
{
	public static class FirmwareDatabase
	{
		...
		{
			Dictionary<string, FirmwareFile> filesByHash = new Dictionary<string, FirmwareFile>();
			List<FirmwareOption> options = new List<FirmwareOption>();
			...
			void Option(string systemId, string id, in FirmwareFile ff, FirmwareOptionStatus status = FirmwareOptionStatus.Acceptable)
				=> options.Add(new FirmwareOption(new FirmwareID(systemId, id), ff.Hash, ff.Size, ff.IsBad ? FirmwareOptionStatus.Bad : status));

			void Firmware(string systemId, string id, string desc)
				=> records.Add(new FirmwareRecord(new FirmwareID(systemId, id), desc));
